...
- **제작자 추천**: 추천 결과에 제작자가 직접 골라둔 맛집이 포함되면 "제작자 추천" 리본이 붙습니다. 점수 계산 로직 자체는 건드리지 않고, 계산된 결과 위에 개인적인 추천을 얹는 방식입니다.
- **실제 맛집 데이터 35곳**: 상호명·주소·영업시간·가격은 다이닝코드·카카오맵 매장 메뉴판·식신 매거진을 직접 확인해 반영 ([상세 출처 및 한계는 아래 참고](#데이터-출처-및-한계))
