...
- **추천 엔진**: 답변을 기반으로 각 맛집에 점수를 매겨 상위 3곳을 반환 (프론트엔드 클라이언트 로직과 백엔드 API가 동일한 규칙을 공유). 카테고리 표기가 달라도 같은 음식(예: "중식 / 짬뽕"과 "중식 / 퓨전짬뽕", "일식 / 돈카츠"와 "한식 / 경양식 돈까스")이면 실제 메뉴 기준(`foodType`)으로 묶어 Top 3에 중복 없이 서로 다른 음식이 나오도록 합니다.
