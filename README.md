...
- **밸런스 게임 퀴즈**: 진행바가 있는 3단계 질문 (15분 초스피드 vs 1시간 여유, 6천원 이하 vs 1만원 이상, 정문 근처 vs 사하10 탑승장 근처)
...
- **제작자 추천**: 추천 결과에 동아분식·리코리코·봉대박 스파게티·다맛·고수 숯불고기주는 냉면&밀면·짬뽕관·밉상짬뽕이 뜨면 "제작자 추천" 리본이 붙습니다. 점수 계산 로직 자체는 건드리지 않고, 계산된 결과 위에 개인적인 추천을 얹는 방식입니다.
- **실제 맛집 데이터 37곳**: 상호명·주소·영업시간·가격은 다이닝코드·카카오맵 매장 메뉴판·식신 매거진을 직접 확인해 반영 ([상세 출처 및 한계는 아래 참고](#데이터-출처-및-한계))
...
- **가격**: 대부분 해당 지점의 실제 카카오맵 메뉴판이나 방문자 후기의 실측 주문 내역을 기준으로 합니다. 다음 예외는 `priceRange`에 사유를 명시해뒀습니다.
  - **짜가후라이드치킨**: 확인된 가격이 2인 이상 세트 가격뿐이라, 1인 기준이 아닙니다.
  - **하이뽕 동아대점**: 동아대점 자체 메뉴판이 공개돼 있지 않아, 같은 프랜차이즈 본점(장림본점)의 확정 메뉴판 가격을 대신 사용했습니다.
  - **밉상짬뽕**: 2026년 신규 개업 매장이라 단품 가격은 아직 확인하지 못했고, 배달 세트 가격만 반영했습니다.
